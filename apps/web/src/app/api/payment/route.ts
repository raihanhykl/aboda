...
// Initialize Snap instance with server-side serverKey only
const snap = new Snap({
  isProduction: false,
  serverKey: 'SB-Mid-server-p8qMfLxRBmt85nq7t9go50wU', // Replace with your actual server key
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Log the shipping cost to verify
    console.log(shippingCost, 'ini shipping cost di route.ts');

    // Calculate the total price (gross amount)
    ...
    const parameter: Midtrans.TransactionRequestBody = {
      transaction_details: {
        ...
        gross_amount: totalPrice, // Total transaction amount
      },
      // item_details: [
      //   {
      //     id: id, // Product ID
      ...
      // ],
      // customer_details: {
      //   first_name: 'Customer', // Example customer data
      //   email: 'customer@example.com',
      // },
    };

    // Create a transaction token using Midtrans Snap
    const transaction = await snap.createTransaction(parameter);

    console.log(transaction.token, 'ini token');
